...
public class Request {
	...
	private static final String MY_ACCOUNTS = "me/accounts"; //[NJ] Added to retrieve my pages
	...
	public static Request newMyAccountsRequest(Session session, final GraphUserCallback callback) {
        Callback wrapper = new Callback() {
            @Override
            public void onCompleted(Response response) {
                if (callback != null) {
        ...
        return new Request(session, MY_ACCOUNTS, null, HttpMethod.GET, wrapper);
    }

    public static Request newMyPageRequest(Session session, String graphPath,final GraphUserCallback callback) {
        Callback wrapper = new Callback() {
            @Override
            public void onCompleted(Response response) {
                if (callback != null) {
        ...
        return new Request(session, graphPath, null, HttpMethod.GET, wrapper);
    }
     ...
     *            the text of the status update
     * @param published
     *            a falg to indicate whether it is published or unpublished post
     * @param callback
     *            a callback that will be called when the request is completed to handle success or error conditions
     * @return a Request that is ready to execute
     */
    public static Request newStatusUpdateRequest(Session session, String graphPath, String message, boolean published, Callback callback) {

        Bundle parameters = new Bundle();
        parameters.putString("message", message);

        parameters.putBoolean("published", published);

		if(published == false){
			parameters.putLong("scheduled_publish_time", System.currentTimeMillis()/1000+600000);
		}
 

        return new Request(session, graphPath, parameters, HttpMethod.POST, callback);
    }
